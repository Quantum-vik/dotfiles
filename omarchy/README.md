...
backgrounds/ <theme>/  <theme>.txt                -> ~/.config/omarchy/backgrounds/<theme>/ (wallpapers + where they came from)
...
`backgrounds/<theme>/` holds the wallpapers for a theme and the `backgrounds` step of `install.sh` links each one
into `~/.config/omarchy/backgrounds/<theme>/` — the folder Omarchy treats as your own backgrounds for that theme,
cycled together with the theme's own by `omarchy-theme-bg-next`. Set one directly with
`omarchy-theme-bg-set <path>`. They are linked, not copied, so dropping a picture into the repo folder puts it on
the machine without rerunning anything.

`backgrounds/<theme>.txt` records where each image came from and what was done to it: `resize:WxH` for one bigger
than any screen here, `cropbottom:N` where a stock-site watermark was cut off the bottom and the picture scaled
back to full height rather than stretched. The images are fan art and belong to their artists.
...
- A 7680x4320 wallpaper costs quickshell roughly 120 MB of memory per screen, on two 1080p panels that gain
  nothing from it. Hence the 1920x1080 copy of the Going Merry, which is the one the machine uses.
