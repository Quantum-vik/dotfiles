...
| `fan` | HP laptops only: a **Fan** toggle in Quick Settings with Auto / Quiet / Max Cooling, switchable with no password (`fan/`). Skipped on other machines |
...
The firmware offers only two fan settings through `hp-wmi`: auto (`pwm1_enable=2`) and full speed (`0`).
It rejects manual mode, and there is no `pwm1` duty value, so no tool can set a percentage. The single fan
is either off or about 3000 RPM, and waits for the CPU to cool to about 48 °C before stopping. The `fan` step
turns those two settings into three modes:
...
| Max Cooling | Fan forced to full speed |

- `/usr/local/sbin/fan-mode status|auto|quiet|max` does the work, and also runs from a terminal with `pkexec`.
